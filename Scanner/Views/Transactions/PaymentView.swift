...
struct PaymentView: View {
    ...
    private var paymentMethod: some View {
        ...
        let maxWidth: CGFloat = options.map { option in
            (option.name as NSString).size(withAttributes: [.font: UIFont.systemFont(ofSize: UIFont.systemFontSize)]).width
        }.max() ?? 0
        ...
        return VStack(alignment: .leading) {
            ForEach(options) { option in
                HStack(spacing: 0) {
                    ...
                    if selectedMethod?.id == option.id {
                        TextField("Enter value", value: $inputValue,formatter: formatter())
                            ...
                            .background(Color(uiColor: UIColor(red: 203/255, green: 203/255, blue: 203/255, alpha: 1)))
                            .clipShape(RoundedCornersShape(corners: [.topRight, .bottomRight], radius: 25))
                ...
                }.background(selectedMethod?.id == option.id ? Color(uiColor: UIColor(red: 203/255, green: 203/255, blue: 203/255, alpha: 1)): Color(uiColor: UIColor(red: 233/255, green: 233/255, blue: 233/255, alpha: 1)))
                    .clipShape(RoundedCornersShape(corners: [.topRight, .bottomRight], radius: 25))
            }
            Spacer()
        }.padding(.vertical)
    ...
    private var countingView : some View{
        return VStack(alignment:.leading){
            HStack{
                ...
                Spacer()
                Text("$\(sumUp(groupedItems: groupItems(selectedItems: selectedItems)))").foregroundStyle(.gray).font(.title)
            }
            Divider()
            HStack{
                Text("Payment").foregroundStyle(.gray)
                Spacer()
                Text("\(selectedMethod?.name ?? "Unknown payment")").foregroundStyle(selectedMethod?.color ?? Color.gray)
            }
            Divider()
            HStack{
            ...
            HStack{
                ...
                Spacer()
                Text("$\(inputValue - sumUp(groupedItems: groupItems(selectedItems: selectedItems)))").foregroundStyle(.gray)
            }
            Spacer()
        }.padding()
    }
